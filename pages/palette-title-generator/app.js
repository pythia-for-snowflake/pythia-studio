...
const S={
  ...
  tfi:0, bfi:0,
  titleText:'Snowflake Insights',
  titleWeight:700, titleSize:24, letterSpacing:-0.02, outlineOn:false, outlineSize:2,
...
function twURL(e){return`https://cdn.jsdelivr.net/npm/@twemoji/svg@latest/${emojiCPs(e).join('-')}.svg`;}
...
function vendorURL(e,v){return v==='twemoji'?twURL(e):v==='noto'?notoURL(e):omURL(e);}
    ...
    <div style="display:flex;flex-direction:column;gap:28px;padding-top:32px">
      <div style="display:flex;gap:24px;align-items:stretch">
        <div style="position:relative;border:1.5px solid #e8e8e8;border-radius:10px;padding:20px 18px 16px;flex:1;margin-top:7px">
          ...
          <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:5px">${presetHtml}</div>
        </div>
...
function buildFontPop(closeHtml){
  ...
  const sliderRow=`<div style="display:flex;align-items:center;gap:16px;flex-wrap:wrap;margin-top:14px">
    ...
    ${mkSlider('size',S.titleSize,14,48,2,'px','S.titleSize=+this.value;render()',70)}
  ...
  const typoRows=[
    {fn:'st.title()',    fam:bf.v, sz:28, fw:700,           sp:0,              color:titleColor, outline:false,      text:S.titleText||'Snowflake Insights'},
...
function buildLogoPop(closeHtml){
  ...
  if(S.logoMode==='text'){
    ...
    const vendorCards=['native','twemoji','noto','openmoji'].map(v=>{
      const labels={native:'Native',twemoji:'Twemoji',noto:'Noto',openmoji:'OpenMoji'};
      const subs={native:'your OS',twemoji:'Twitter / X',noto:'Google',openmoji:'Open-source'};
      const on=S.emojiVendor===v;
      const imgHtml=v==='native'?`<span style="font-size:30px;line-height:1">${S.selectedEmoji}</span>`
        :`<img src="${vendorURL(S.selectedEmoji,v)}" style="width:30px;height:30px;object-fit:contain" onerror="this.style.display='none'">`;
    ...
    pickerSection=`<div style="display:grid;grid-template-columns:1.4fr 1fr;gap:16px;align-items:start">
      ...
      <div>
        ...
        <div style="padding:8px 10px;background:#fffbeb;border:1px solid #fde68a;color:#92400e;border-radius:6px;font-size:11px;line-height:1.5">
          💡 For shared apps pick <strong>Twemoji</strong> or <strong>Noto</strong>.
        </div>
      </div>
    </div>`;
  } else if(S.logoMode==='material'){
    ...
    if(isDisk){
      ...
      srcInput=`<input value="${S.logoImageUrl}" oninput="S.logoImageUrl=this.value;render()" placeholder="https://…/logo.png"
  ...
  <div style="display:grid;grid-template-columns:1.4fr 1fr;gap:20px;margin-bottom:14px;padding-top:32px;align-items:start">
    <div style="display:flex;flex-direction:column;gap:25px">
      <div style="display:flex;gap:6px;align-items:center">
        ...
        <input value="${S.titleText}" oninput="S.titleText=this.value;render()" placeholder="My App"
...
async function downloadPng(kind){
  ...
  const tf=TITLE_FONTS[S.tfi];const SCALE=5;const fs=S.titleSize*SCALE;const fam=tf.v.split(',')[0].replace(/'/g,'').trim();
  let emojiImg=null;
  if(S.emojiVendor!=='native'&&S.selectedEmoji){
  ...
  if(kind==='icon'){
    const side=fs+20*SCALE,canvas=document.createElement('canvas');canvas.width=side;canvas.height=side;
    const ctx=canvas.getContext('2d'),cx=side/2,cy=side/2;
    if(emojiImg){ctx.drawImage(emojiImg,cx-fs/2,cy-fs/2,fs,fs);}
    else if(S.selectedEmoji){ctx.font=`${fs}px Apple Color Emoji,Segoe UI Emoji,Noto Color Emoji,sans-serif`;ctx.textBaseline='middle';ctx.textAlign='center';ctx.fillText(S.selectedEmoji,cx,cy);}
  ...
  const tw=measCtx.measureText(S.titleText||'App').width;
  const hasI=S.selectedEmoji||emojiImg;const eW=hasI?fs:0,gapPx=hasI?S.emojiGap*SCALE:0;
  const padX=20*SCALE,padY=14*SCALE,canvas=document.createElement('canvas');
  canvas.width=eW+gapPx+tw+padX*2;canvas.height=fs+padY*2;
  const ctx=canvas.getContext('2d'),cy=canvas.height/2;
  let ex=padX,tx=padX;
  if(hasI){if(S.emojiPos==='before'){ex=padX;tx=padX+eW+gapPx;}else{tx=padX;ex=padX+tw+gapPx;}}
  if(emojiImg){ctx.drawImage(emojiImg,ex,cy-fs/2,fs,fs);}
  else if(S.selectedEmoji){ctx.font=`${fs}px Apple Color Emoji,Segoe UI Emoji,Noto Color Emoji,sans-serif`;ctx.textBaseline='middle';ctx.textAlign='left';ctx.fillText(S.selectedEmoji,ex,cy);}
  ctx.font=`${S.titleWeight} ${fs}px ${fam},sans-serif`;ctx.letterSpacing=(S.letterSpacing*fs)+'px';
  ctx.fillStyle=sigColor;ctx.textBaseline='middle';ctx.textAlign='left';ctx.fillText(S.titleText||'App',tx,cy);
  trigDl(canvas,`wide-${S.emojiVendor}.png`);
}
function trigDl(canvas,name){const a=document.createElement('a');a.download=name;a.href=canvas.toDataURL('image/png');a.click();}
